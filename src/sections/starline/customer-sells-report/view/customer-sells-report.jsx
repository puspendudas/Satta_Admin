...
export default function CustomerSellsReportPage() {
  ...
  const downloadReportAsImage = async () => {
    if (reportRef.current) {
      ...
      link.download = `StarlineCustomerSellsReport_${dayjs().format('YYYY-MM-DD')}.png`;
      link.click();
    }
  };
  ...
  const categories = [
    ...
    { key: 'even-odd-digit', title: 'Even Odd Digit' },
  ];

  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Starline Customer Sells Report</Typography>
        <Button
          variant="contained"
          size="large"
          color="primary"
