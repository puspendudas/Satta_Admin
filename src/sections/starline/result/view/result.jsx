...
import {
  ...
  fetchStarlineBetResults,
  fetchStarlineResultMarkets,
...
export default function ResultPage() {
  ...
  const fetchResultData = useCallback(async () => {
    ...
    try {
      const resultList = await fetchStarlineBetResults(state.resultListData);
  ...
  const fetchMarketData = useCallback(async () => {
    setState((prevState) => ({ ...prevState, loading: true }));
    try {
      const marketData = await fetchStarlineResultMarkets(state.declareResultData.result_date);
      const marketBidData = await fetchStarlineMarkets();
  ...
  const handleToggle = () => {
    ...
    fetchStarlineBetResults(state.resultListData).then((resultList) =>
      setState((prevState) => ({ ...prevState, resultData: resultList }))
    );
  };
  ...
  const handleTableDateSelect = async (e) => {
    setState((prevState) => ({
      ...prevState,
      selectTableDate: e,
      ...
      >
        ...
        >
          <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="en-gb">
            <DemoContainer
              sx={{ width: '100%', maxWidth: { xs: '100%', sm: 400 }, mb: { xs: 2, sm: 0 } }}
          ...
          <Autocomplete
            ...
            freeSolo
            sx={{ width: '100%', maxWidth: { xs: '100%', sm: 440 }, mb: { xs: 2, sm: 0 } }}
            id="free-solo-2-demo"
            disableClearable
