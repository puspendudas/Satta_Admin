...
export default function CustomerSellsReportPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const marketData = await fetchGaliMarkets();
  ...
  const downloadReportAsImage = async () => {
    if (reportRef.current) {
      ...
      link.download = `GaliDiSawarCustomerSellsReport_${dayjs().format('YYYY-MM-DD')}.png`;
      link.click();
    }
  };

  const categories = [
    { key: 'left-digit', title: 'Left Digit' },
    { key: 'right-digit', title: 'Right Digit' },
    { key: 'jodi-digit', title: 'Jodi Digit' },
  ];

  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Gali-Di-Sawar Customer Sells Report</Typography>
        <Button
          variant="contained"
          size="large"
          color="primary"
      ...
      >
        <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="en-gb">
          <DemoContainer components={['DatePicker']} sx={{ width: '100%', maxWidth: { xs: '100%', sm: 200 }, mb: { xs: 2, sm: 0 } }}>
            <DatePicker
              label="Select Date"
        ...
        <Autocomplete
          freeSolo
          sx={{ width: '100%', maxWidth: { xs: '100%', sm: 200 }, mb: { xs: 2, sm: 2 } }}
          id="free-solo-2-demo"
          disableClearable
          options={markets}
