...
import {
  ...
  fetchGaliMarkets,
  fetchGaliBetResults,
  fetchGaliResultMarkets,
} from 'src/_mock/market';
...
export default function ResultPage() {
  ...
  const { revartResultApi, marketStatusApi, declareGaliResultApi } = useApiResultCalls();
  ...
  const fetchResultData = useCallback(async () => {
    ...
    try {
      const resultList = await fetchGaliBetResults(state.resultListData);
  ...
  const fetchMarketData = useCallback(async () => {
    setState((prevState) => ({ ...prevState, loading: true }));
    try {
      const marketData = await fetchGaliResultMarkets(state.declareResultData.result_date);
      const marketBidData = await fetchGaliMarkets();
  ...
  useEffect(() => {
    fetchMarketData();
    fetchResultData();
  }, [fetchMarketData, fetchResultData]);

  const handleToggle = useCallback(() => {
    fetchMarketData();
    fetchGaliBetResults(state.resultListData).then((resultList) => {
      setState((prevState) => ({ ...prevState, resultData: resultList }));
    });
  ...
  const handleDeclareResult = useCallback(async () => {
    try {
      const data = await declareGaliResultApi(state.declareResultData);
  ...
  const isFormComplete = useMemo(() => {
    const { market_id, left_digit, right_digit } = state.declareResultData;
    return market_id && left_digit && right_digit;
  }, [state.declareResultData]);
            ...
            onChange={(event, newValue) => {
              ...
              if (newValue) {
                setState((prevState) => ({
                  ...
                  declareResultData: {
                    ...prevState.declareResultData,
                    left_digit: newValue.split('')[0],
                    right_digit: newValue.split('')[1],
                  },
                }));
              }
            }}
