...
export default function PuspaModal({ isOpen, onClose, title, isEditMode, editData }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { name, name_hindi, open_time, close_time } = formData;
            ...
            >
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DemoContainer
                  sx={{
                    width: '100%',
                    mt: { xs: 2, sm: 0 }, // Add margin-top for spacing on small screens
                  }}
                  components={['TimePicker']}
                >
                  <TimePicker
                    label="Close Time"
                    sx={{ width: '100%' }}
                    value={dayjs(`1970-01-01T${formData.close_time}`)}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        close_time: e.$d.toLocaleTimeString(),
                      })
                    }
                    viewRenderers={{
                      hours: renderTimeViewClock,
