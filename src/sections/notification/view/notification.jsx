...
import { notificationGetApi } from 'src/apis/notificationApi';
...
export default function NoticePage() {
  ...
  useEffect(() => {
    const fetchNoticeData = async () => {
      ...
      try {
        ...
        await notificationGetApi().then((noticeList) => {
          SetNoticeData(noticeList?.data.reverse()); // Store the users in state
        });
      } catch (err) {
  ...
  const handleToggle = () => {
    notificationGetApi().then((noticeList) => SetNoticeData(noticeList?.data.reverse()));
  };
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Notification Mangment</Typography>
