...
export default function PuspaCard({ userData, openMpinModal, openAddModal, openWithdrawModal }) {
  ...
  const { user_name, createdAt, updatedAt } = userData;
  ...
  const renderInformation = (
    <Grid container sx={{ p: 4 }} spacing={3}>
      <Grid xs={12} sm={6}>
        ...
        <Typography>OTP: {userData.verified ? 'Yes' : 'No'}</Typography>
        ...
        <Typography>Transfer: {userData.transfer ? 'Enabled' : 'Disabled'}</Typography>
      </Grid>
      {/* <Grid xs={12} sm={4}>
  ...
  const renderDate = (
    ...
    >
      ...
      <br />
      Last Seen : {formatDate(updatedAt)}
    </Typography>
  );
